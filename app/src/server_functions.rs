...
#[server(GenerateLLMRule)]
pub async fn generate_llm_rule(
    api_key: String,
    model: String,
    system_prompt: Option<String>,
    tracks: Vec<Track>,
) -> Result<::scrobble_scrubber::rewrite::RewriteRule, ServerFnError> {
    ...
    if api_key.trim().is_empty() {
        return Err(ServerFnError::new("API key is required"));
    }

    if tracks.is_empty() {
        return Err(ServerFnError::new("No tracks provided"));
    }

    // Take a sample of tracks for analysis (limit to avoid token limits)
    let sample_tracks: Vec<_> = tracks.into_iter().take(10).collect();

    // Create the provider and enable rule focus mode
    let mut provider = OpenAIScrubActionProvider::new(
        api_key,
        Some(model),
        system_prompt,
        Vec::new(), // Empty rewrite rules for LLM generation
    )
    .to_server_error("Failed to create OpenAI provider")?;
    provider.enable_rule_focus_mode();

    // Analyze tracks to get rule suggestions
    let suggestions = provider
        .analyze_tracks(&sample_tracks, None, None)
        .await
        .to_server_error("Failed to analyze tracks with LLM")?;

    // Extract the first rule suggestion
    for (_track_index, track_suggestions) in suggestions {
        for suggestion in track_suggestions {
            if let ::scrobble_scrubber::scrub_action_provider::ScrubActionSuggestion::ProposeRule { rule, .. } = suggestion {
                return Ok(rule);
            }
        }
    }

    Err(ServerFnError::new("No rule suggestions generated by LLM"))
}

#[server(GenerateLLMRuleForTrack)]
pub async fn generate_llm_rule_for_track(
    api_key: String,
    model: String,
    system_prompt: Option<String>,
    target_track: Track,
    ...
    // Create tracks list with target track first, followed by context
    let mut tracks_for_analysis = vec![target_track];
    tracks_for_analysis.extend(context_tracks.into_iter().take(9)); // Limit context to 9 additional tracks
    ...
    // Analyze tracks to get rule suggestions, focusing on the first track (target)
    let suggestions = provider
        .analyze_tracks(&tracks_for_analysis, None, None)
        .await
        .to_server_error("Failed to analyze tracks with LLM")?;
